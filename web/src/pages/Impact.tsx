...
import { api, type ImpactData, type ProofData } from "@/lib/api";

export function Impact() {
  const [impact, setImpact] = useState<ImpactData | null>(null);
  const [proof, setProof] = useState<ProofData | null>(null);
  useEffect(() => { Promise.all([api.impact(), api.proof()]).then(([i, p]) => { setImpact(i); setProof(p); }).catch(() => undefined); }, []);
  ...
  return <div className="mx-auto max-w-3xl space-y-6">
    ...
    {proof.coverage?.status !== "verified" && <section className="rounded-md border border-status-pending/40 bg-status-pending/10 p-4 text-sm"><p className="font-semibold">Coverage is city-level fallback</p><p className="mt-1 text-muted-foreground">{proof.coverage?.pilot_area ?? "Sub-city pilot"} is shown as a pilot label only. The official sub-city polygon has not yet been vendored, so map and scoreboard labels must not be treated as verified routing.</p>{proof.coverage?.boundary_source && <a className="mt-2 inline-block text-xs text-primary underline-offset-4 hover:underline" href={proof.coverage.boundary_source} target="_blank" rel="noreferrer">View boundary source</a>}</section>}
